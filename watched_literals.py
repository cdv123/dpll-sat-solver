...
def dpll_sat_solve_wrapper(partial_assignment,units,watch_literals,literals):
    ...
    units2 = units[:]
    partial_assignment2,units2,watch_literals = set_var(partial_assignment2, watch_literals,var,units2)
    if dpll_sat_solve_wrapper(partial_assignment,units2,watch_literals,literals) == False:
        partial_assignment2 = partial_assignment
        units2 = units[:]
    else:
        return partial_assignment2
    partial_assignment2,units2,watch_literals = set_var(partial_assignment2, watch_literals,-1*var,units2)
    if dpll_sat_solve_wrapper(partial_assignment2,units2,watch_literals,literals) == False:
        partial_assignment2 = partial_assignment
        units2 = units
        return False
    return partial_assignment2
...
def unit_propagate(partial_assignment,units,watch_literals):
    ...
    while units != []:
        partial_assignment,units,watch_literals = set_var(partial_assignment,watch_literals,units[-1],units)
        if units == False:
...
def set_var(partial_assignment,watch_literals,var,units):
    ...
    if partial_assignment[abs(var)] != 0 and partial_assignment[abs(var)]*var != var:
        return partial_assignment,False,watch_literals
    ...
    for clause in watch_literals[-var]:
        ...
        for literal in clause:
            ...
            if literal != -var and partial_assignment[abs(literal)] == 0 and isIn == False:
                ...
                break
            if literal == clause[-1]:
                if partial_assignment[abs(potential_units[0])] == 0:
